...
napi_value ParseParametersMedianBlur(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersMedianBlur.");
    ...
    Common::GetInt32Value(env, argv[PARAM1], paras.ksize);
    // callback
    if (argc >= PARAM3) {
...
void AsyncCompleteCallbackMedianBlur(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackMedianBlur.");
...
{
    OPENCV_LOGI("Enter n_medianBlur.");

    InputParametersInfo params;
    if (ParseParametersMedianBlur(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "medianBlur", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                cv::medianBlur(image, outMat, asynccallbackinfo->params.ksize);
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/medianBlur.jpg", outMat);
        ...
        AsyncCompleteCallbackMedianBlur,
