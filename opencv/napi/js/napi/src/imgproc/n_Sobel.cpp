...
napi_value ParseParametersSobel(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersSobel.");
    ...
    Common::GetInt32Value(env, argv[PARAM4], paras.ksize);
    Common::GetDoubleValue(env, argv[PARAM5], paras.scale);
    Common::GetDoubleValue(env, argv[PARAM6], paras.delta);
    Common::GetInt32Value(env, argv[PARAM7], paras.borderType);
    // callback
    if (argc >= PARAM9) {
...
void AsyncCompleteCallbackSobel(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackSobel.");
...
{
    OPENCV_LOGI("Enter n_Sobel.");

    InputParametersInfo params;
    if (ParseParametersSobel(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "Sobel", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                cv::Sobel(image, outMat, asynccallbackinfo->params.ddepth, asynccallbackinfo->params.dx,
                          asynccallbackinfo->params.dy, asynccallbackinfo->params.ksize,
                          asynccallbackinfo->params.scale, asynccallbackinfo->params.delta,
                          asynccallbackinfo->params.borderType);
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/Sobel.jpg", outMat);
