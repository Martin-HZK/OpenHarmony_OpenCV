...
napi_value ParseParametersPyrDown(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersPyrDown.");
...
void AsyncCompleteCallbackPyrDown(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackPyrDown.");
...
{
    OPENCV_LOGI("Enter n_pyrDown.");

    InputParametersInfo params;
    if (ParseParametersPyrDown(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "pyrDown", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                cv::pyrDown(image, outMat, dsize, asynccallbackinfo->params.borderType);
