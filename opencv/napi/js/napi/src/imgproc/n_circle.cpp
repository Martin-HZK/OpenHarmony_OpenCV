...
napi_value ParseParametersCircle(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersCircle.");
    ...
    Common::GetInt32Value(env, argv[PARAM2], paras.radius);
    Common::GetScalarInfo(env, argv[PARAM3], paras.scalar);
    Common::GetInt32Value(env, argv[PARAM4], paras.thickness);
    // callback
    if (argc >= PARAM6) {
...
void AsyncCompleteCallbackCircle(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackCircle.");
...
{
    OPENCV_LOGI("Enter n_circle.");

    InputParametersInfo params;
    if (ParseParametersCircle(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "circle", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                Scalar scalar(asynccallbackinfo->params.scalar.v0, asynccallbackinfo->params.scalar.v1,
                              asynccallbackinfo->params.scalar.v2);
                cv::circle(outMat, center, asynccallbackinfo->params.radius, scalar,
                           asynccallbackinfo->params.thickness);
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/circle.jpg", outMat);
