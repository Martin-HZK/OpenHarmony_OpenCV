...
napi_value ParseParametersLine(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersLine.");
    ...
    Common::GetPointInfo(env, argv[PARAM1], paras.pt1);
    Common::GetPointInfo(env, argv[PARAM2], paras.pt2);
...
void AsyncCompleteCallbackLine(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackLine.");
...
napi_value n_line(napi_env env, napi_callback_info info)
{
    ...
    InputParametersInfo params;
    if (ParseParametersLine(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "line", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                Point pt1(asynccallbackinfo->params.pt1.x, asynccallbackinfo->params.pt1.y);
                Point pt2(asynccallbackinfo->params.pt2.x, asynccallbackinfo->params.pt2.y);

                Scalar color(asynccallbackinfo->params.scalar.v0, asynccallbackinfo->params.scalar.v1,
                             asynccallbackinfo->params.scalar.v2);

                cv::line(outMat, pt1, pt2, color, asynccallbackinfo->params.thickness);
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/line.jpg", outMat);
