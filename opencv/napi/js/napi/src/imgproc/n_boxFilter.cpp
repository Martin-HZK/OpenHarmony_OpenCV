...
napi_value ParseParametersBoxFilter(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersBoxFilter.");
...
void AsyncCompleteCallbackBoxFilter(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackBoxFilter.");
...
{
    OPENCV_LOGI("Enter n_boxFilter.");

    InputParametersInfo params;
    if (ParseParametersBoxFilter(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "boxFilter", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                cv::boxFilter(image, outMat, asynccallbackinfo->params.ddepth, ksize, anchor,
                              asynccallbackinfo->params.normalize, asynccallbackinfo->params.borderType);
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/boxFilter.jpg", outMat);
        ...
        AsyncCompleteCallbackBoxFilter,
