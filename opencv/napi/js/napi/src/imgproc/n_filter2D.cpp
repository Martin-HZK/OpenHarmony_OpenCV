...
napi_value GetDoubleArrayInfo(const napi_env &env, const napi_value &value, std::vector<double> &kernel)
{
    ...
    for (int index = 0; index < array_len; index++) {
        ...
        Common::GetDoubleValue(env, element, tmp);
        kernel.push_back(tmp);
    }
    return Common::NapiGetNull(env);
}

napi_value ParseParametersFilter2D(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersFilter2D.");
    ...
    GetDoubleArrayInfo(env, argv[PARAM2], paras.kernel);
    Common::GetPointInfo(env, argv[PARAM3], paras.anchor);
    Common::GetDoubleValue(env, argv[PARAM4], paras.delta);
...
void AsyncCompleteCallbackFilter2D(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackFilter2D.");
...
{
    OPENCV_LOGI("Enter n_filter2D.");

    InputParametersInfo params;
    if (ParseParametersFilter2D(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "filter2D", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                cv::filter2D(image, outMat, asynccallbackinfo->params.ddepth, asynccallbackinfo->params.kernel,
                             anchor, asynccallbackinfo->params.delta, asynccallbackinfo->params.borderType);
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/filter2D.jpg", outMat);
