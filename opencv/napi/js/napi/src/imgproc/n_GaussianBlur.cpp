...
napi_value ParseParametersGaussianBlur(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersGaussianBlur.");
    ...
    Common::GetSizeInfo(env, argv[PARAM1], paras.ksize);
    Common::GetDoubleValue(env, argv[PARAM2], paras.sigmaX);
    Common::GetDoubleValue(env, argv[PARAM3], paras.sigmaY);
...
void AsyncCompleteCallbackGaussianBlur(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackGaussianBlur.");
...
{
    OPENCV_LOGI("Enter n_GaussianBlur.");

    InputParametersInfo params;
    if (ParseParametersGaussianBlur(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "GaussianBlur", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                cv::GaussianBlur(image, outMat, ksize, asynccallbackinfo->params.sigmaX,
                                 asynccallbackinfo->params.sigmaY, asynccallbackinfo->params.borderType);
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/GaussianBlur.jpg", outMat);
        ...
        AsyncCompleteCallbackGaussianBlur,
