...
napi_value ParseParametersBlur(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersBlur.");
    ...
    Common::GetPointInfo(env, argv[PARAM2], paras.anchor);
    Common::GetInt32Value(env, argv[PARAM3], paras.borderType);
    // callback
    if (argc >= PARAM5) {
...
void AsyncCompleteCallbackBlur(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackBlur.");
...
{
    OPENCV_LOGI("Enter n_blur.");

    InputParametersInfo params;
    if (ParseParametersBlur(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "blur", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                cv::blur(image, outMat, ksize, anchor, asynccallbackinfo->params.borderType);

#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/blur.jpg", outMat);
