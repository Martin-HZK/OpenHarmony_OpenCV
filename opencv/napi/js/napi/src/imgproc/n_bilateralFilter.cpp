...
napi_value ParseParametersBilateralFilter(const napi_env &env,
                                          const napi_callback_info &info,
                                          InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersBilateralFilter.");
    ...
    Common::GetInt32Value(env, argv[PARAM1], paras.d);
    Common::GetDoubleValue(env, argv[PARAM2], paras.sigmaColor);
    Common::GetDoubleValue(env, argv[PARAM3], paras.sigmaSpace);
    Common::GetInt32Value(env, argv[PARAM4], paras.borderType);
    // callback
    if (argc >= PARAM6) {
...
void AsyncCompleteCallbackBilateralFilter(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackBilateralFilter.");
...
{
    OPENCV_LOGI("Enter n_bilateralFilter.");

    InputParametersInfo params;
    if (ParseParametersBilateralFilter(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "bilateralFilter", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                Mat outMat;
                image.copyTo(outMat);
                cv::bilateralFilter(image, outMat, asynccallbackinfo->params.d,
                                    asynccallbackinfo->params.sigmaColor, asynccallbackinfo->params.sigmaSpace,
                                    asynccallbackinfo->params.borderType);
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/bilateralFilter.jpg", outMat);
        ...
        AsyncCompleteCallbackBilateralFilter,
