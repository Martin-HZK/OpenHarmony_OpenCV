...
napi_value ParseParametersScharr(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersScharr.");
    ...
    Common::GetInt32Value(env, argv[PARAM2], paras.dx);
    Common::GetInt32Value(env, argv[PARAM3], paras.dy);
    Common::GetDoubleValue(env, argv[PARAM4], paras.scale);
    Common::GetDoubleValue(env, argv[PARAM5], paras.delta);
    Common::GetInt32Value(env, argv[PARAM6], paras.borderType);
    // callback
    if (argc >= PARAM8) {
        NAPI_CALL(env, napi_typeof(env, argv[PARAM7], &valuetype));
        if (valuetype != napi_function) {
            OPENCV_LOGE("Wrong argument type. Function expected.");
            return nullptr;
        }
        napi_create_reference(env, argv[PARAM7], 1, &paras.callback);
    }

    return Common::NapiGetNull(env);
}

void AsyncCompleteCallbackScharr(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackScharr.");
...
{
    OPENCV_LOGI("Enter n_Scharr.");

    InputParametersInfo params;
    if (ParseParametersScharr(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "Scharr", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                Mat outMat;
                image.copyTo(outMat);
                cv::Scharr(image, outMat, asynccallbackinfo->params.ddepth, asynccallbackinfo->params.dx,
                           asynccallbackinfo->params.dy, asynccallbackinfo->params.scale,
                           asynccallbackinfo->params.delta, asynccallbackinfo->params.borderType);
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/Scharr.jpg", outMat);
