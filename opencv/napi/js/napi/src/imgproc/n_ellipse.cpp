...
napi_value ParseParametersEllipse(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersEllipse.");
    ...
    Common::GetPointInfo(env, argv[PARAM1], paras.center);
    Common::GetSize2fInfo(env, argv[PARAM2], paras.axes);
    Common::GetDoubleValue(env, argv[PARAM3], paras.angle);
    Common::GetScalarInfo(env, argv[PARAM4], paras.color);
    Common::GetInt32Value(env, argv[PARAM5], paras.thickness);
    // argv[1]: callback
    if (argc >= PARAM7) {
...
void AsyncCompleteCallbackEllipse(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackEllipse.");
...
{
    OPENCV_LOGI("Enter n_ellipse.");

    InputParametersInfo params;
    if (ParseParametersEllipse(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "ellipse", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                Point center(asynccallbackinfo->params.center.x, asynccallbackinfo->params.center.y);
                Size2f axes(asynccallbackinfo->params.axes.width, asynccallbackinfo->params.axes.height);
                ...
                e1.center = center;
                e1.size = axes;
                e1.angle = asynccallbackinfo->params.angle;
                cv::ellipse(outMat, e1, color, asynccallbackinfo->params.thickness);

#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/ellipse.jpg", outMat);
