...
napi_value ParseParametersBitwiseAnd(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersBitwiseAnd\n");
...
void AsyncCompleteCallbackBitwiseAnd(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackBitwiseAnd");
    ...
    OPENCV_LOGI("AsyncCompleteCallbackBitwiseAnd end.");
}

napi_value n_bitwise_and(napi_env env, napi_callback_info info)
{
    OPENCV_LOGI("Enter n_bitwise_and");

    InputParametersInfo params;
    if (ParseParametersBitwiseAnd(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "bitwise_and", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                OPENCV_LOGI("Input second mat info: rows=%{public}d, cols=%{public}d, type=%{public}d, buffsize=%{public}d",
                ...
                Mat outMat;
                bitwise_and(image, secImage, outMat);
                
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/bitwise_and.jpg", outMat);
        ...
        AsyncCompleteCallbackBitwiseAnd,
