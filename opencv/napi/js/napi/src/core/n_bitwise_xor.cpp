...
napi_value ParseParametersBitwiseXor(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersBitwiseXor\n");
...
void AsyncCompleteCallbackBitwiseXor(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackBitwiseXor");
    ...
    OPENCV_LOGI("AsyncCompleteCallbackBitwiseXor end.");
}

// 原生：void bitwise_xor(InputArray src1, InputArray src2, OutputArray dst, InputArray mask = noArray());
napi_value n_bitwise_xor(napi_env env, napi_callback_info info)
{
    OPENCV_LOGI("Enter n_bitwise_xor");

    InputParametersInfo params;
    if (ParseParametersBitwiseXor(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "bitwise_xor", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                Mat outMat;
                bitwise_xor(image, secImage, outMat);
                
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/bitwise_xor.jpg", outMat);
        ...
        AsyncCompleteCallbackBitwiseXor,
