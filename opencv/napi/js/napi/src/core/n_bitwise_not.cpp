...
napi_value ParseParametersBitwiseNot(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersBitwiseNot\n");
...
void AsyncCompleteCallbackBitwisNot(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackBitwisNot");
    ...
    OPENCV_LOGI("AsyncCompleteCallbackBitwisNot end.");
}

napi_value n_bitwise_not(napi_env env, napi_callback_info info)
{
    OPENCV_LOGI("Enter n_bitwise_not");

    InputParametersInfo params;
    if (ParseParametersBitwiseNot(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "bitwise_not", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                Mat outMat;
                bitwise_not(image, outMat);
                
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/bitwise_not.jpg", outMat);
        ...
        AsyncCompleteCallbackBitwisNot,
