...
napi_value ParseParametersHconcat(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersHconcat\n");
...
void AsyncCompleteCallbackHconcat(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackHconcat");
    ...
    OPENCV_LOGI("AsyncCompleteCallbackHconcat end.");
}

napi_value n_hconcat(napi_env env, napi_callback_info info)
{
    OPENCV_LOGI("Enter n_hconcat");

    InputParametersInfo params;
    if (ParseParametersHconcat(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "hconcat", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                Mat outMat;
                hconcat(image, secImage, outMat);
                
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/hconcat.jpg", outMat);
