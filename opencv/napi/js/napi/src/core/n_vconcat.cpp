...
napi_value ParseParametersVconcat(const napi_env &env, const napi_callback_info &info, InputParametersInfo &paras)
{
    OPENCV_LOGI("Enter ParseParametersVconcat\n");
...
void AsyncCompleteCallbackVconcat(napi_env env, napi_status status, void *data)
{
    OPENCV_LOGI("Enter AsyncCompleteCallbackVconcat");
    ...
    OPENCV_LOGI("AsyncCompleteCallbackVconcat end.");
}

// 原生：CV_EXPORTS void vconcat(InputArray src1, InputArray src2, OutputArray dst);
napi_value n_vconcat(napi_env env, napi_callback_info info)
{
    OPENCV_LOGI("Enter n_copyMakeBorder");

    InputParametersInfo params;
    if (ParseParametersVconcat(env, info, params) == nullptr) {
    ...
    napi_create_string_latin1(env, "vconcat", NAPI_AUTO_LENGTH, &resourceName);
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, void *data) {
            ...
            if (asynccallbackinfo) {
                ...
                Mat outMat;
                vconcat(image, secImage, outMat);
                
#if defined OPENCV_NAPI_DEBUG
                bool ret = imwrite("/data/storage/el2/base/haps/entry/files/vconcat.jpg", outMat);
